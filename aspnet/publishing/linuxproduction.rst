...
Configure SSL
`````````````
Configure your server to listen to HTTPS traffic on port ``443`` by specifying a valid certificate issued by a trusted Certificate Authority (CA). Harden your security by employing some of the practices suggested below like choosing a stronger cipher and redirecting all traffic over HTTP to HTTPS. Additionally, adding an ``HTTP Strict-Transport-Security`` (HSTS) header ensures all subsequent requests made by the client are over HTTPS only. Do not add the Strict-Transport-Security header or chose an appropriate ``max-age`` if you plan to disable SSL in the future.
